...
write_ampseq = function(ampseq_object, format = 'excel', name = 'wb.xlsx'){
  ...
  if(format == 'excel'){
    ...
    for(temp_slot in c('gt', 'metadata', 'markers', 'loci_performance')){
...
haplotypes_respect_to_reference = function(gt,
                                           markers_extrated,
                                           gene_names = c('PfDHFR',
                                                          'PfMDR1',
                                                          'PfDHPS',
  ...
  if(length(gene_ids) > 1){
    ...
    moi_loci_abd_table = matrix(gt[,grep(paste(gene_names, collapse = "|"), colnames(gt))],
                                ncol = 1,
  ...
  if(plot_haplo_freq){
    ...
    metadata = metadata_extracted[,variables]
...
drug_resistant_haplotypes = function(gt,
  ...
  metadata = metadata_extracted[,c(variables, Longitude, Latitude)]
